...
class IFrameStream extends Transform {
	 ...
	 * @param {object} awsInstances - see kvs-stream.js
	 * @param {object} encoder - It return encoded object if encoder object of bemcoder or string "jpeg" values provided Otherwise returns frame object.
	*/
	constructor(getMediaParameters, awsInstances, encoder = null) {
		super({
			...
			if (blocks[index].keyframe === true) {
				...
				return decoder.decode(packet).then(decResult => {
					if (decResult.frames.length === 0) {
						...
						return this.encoder.encode(decResult.frames).then(jpeg => jpeg).catch(error => {
