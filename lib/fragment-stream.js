	...
	*/
	...
	_transform(fragment, enc, cb) {
		if (fragment.idName === 'Segment') {
			...
			const segmentCluster = fragment.Children.filter(a => a.idName === 'Cluster');
			const tags = this.getTags(segmentTags);
			const tracks = this.getTracks(segmentTracks);
			const blocks = this.getBuffersBlocks(segmentCluster);
			return cb(null, {tags, tracks, blocks});
		}
	...
	getBuffersBlocks(segmentCluster) {
		const blocks = [];
		for (const clusterTag of segmentCluster) {
			const buffer = {};
			for (const clusterChild of clusterTag.Children) {
				if (clusterChild.idName === 'Timecode') {
					buffer.timecode = clusterChild.data;
				} else if (clusterChild.idName === 'SimpleBlock') {
					buffer.payload = clusterChild.payload;
				}
			}
