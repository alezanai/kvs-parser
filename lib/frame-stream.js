						...
						if (segmentTag.Children[0].Children.length > 0) {
							for (const simpleTag of segmentTag.Children[0].Children) {
								if (simpleTag.Children.length === 2) {
									...
									error = new Error(`SimpleTag id: ${simpleTag.id} have ${simpleTag.Children.length} Children`);
								}
							}
						} else {
							error = new Error(`Tag id: ${segmentTag.Children[0].id} does not have any Children`);
						}
					} else {
						error = new Error(`SegmentTag id: ${segmentTag.id} does not have any Children`);
					}
				}
			}
		} else {
